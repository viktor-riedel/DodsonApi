...
class UpdateIcNumberAction
{
    public function handle(Request $request, Car $car, CarPdrPositionCard $card): array
    {
        ...
        //update pricing
        $card->priceCard()->update([
            'pricing_nz_retail' => (int) round(($baseCard?->price_nz_retail * $nzd->rate_to_jpy) / 100 * 100),
            'pricing_nz_wholesale' => (int) round(($baseCard?->price_nz_wholesale * $nzd->rate_to_jpy)  / 100 * 100),
            'pricing_ru_retail' => (int) round(($baseCard?->price_ru_retail * $rub->rate_to_jpy) / 100 * 100),
            'pricing_ru_wholesale' => (int) round(($baseCard?->price_ru_wholesale * $rub->rate_to_jpy) / 100 * 100),
            'pricing_mng_retail' => (int) round(($baseCard?->price_mng_retail * $mnt->rate_to_jpy) / 100 * 100),
            'pricing_mng_wholesale' => (int) round(($baseCard?->price_mng_wholesale * $mnt->rate_to_jpy) / 100 * 100),
